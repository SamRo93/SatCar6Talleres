 ...
 * @author Samuel
 */
public class ClientemostrarDAOImplHibernate extends GenericDAOImplHibernate<Clientemostrar, Integer> implements ClientemostrarDAO {
    ...
    public List<Clientemostrar> busquedaDB(String s, Object dato) throws Exception {
        ...
        return (List<Clientemostrar>) q.list();
    }

    @Override
    public Clientemostrar busquedaId(int id) throws Exception {
        Session session = HibernateUtil.getSessionFactory().openSession();
        session.beginTransaction();
        Query q = session.createQuery("Select doc From Clientemostrar doc where doc.id = '"+String.valueOf(id)+"'");
        session.getTransaction().commit();
        return (Clientemostrar) q.uniqueResult();
    }
